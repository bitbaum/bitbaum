 ...
 */
export function parseQuery(params, spec, opts = {}) {
    ...
    for (const f of spec.facets) {
        ...
        const allowed = f.options
            ? wanted.filter((v) => f.options.includes(v))
            : wanted;
    ...
    const sort = sortRaw !== undefined && spec.sorts.some((s) => s.key === sortRaw) ? sortRaw : spec.defaultSort;
    const dirRaw = readAll(params, DIR_KEY)[0];
    const dir = dirRaw === "asc" || dirRaw === "desc" ? dirRaw : (spec.defaultDir ?? "asc");
    return {
...
export function sameResultSet(a, b) {
    if (a.q !== b.q || a.sort !== b.sort || a.dir !== b.dir)
        return false;
 ...
 */
export function writeQuery(current, next, spec, previous) {
    const out = current instanceof URLSearchParams
        ? new URLSearchParams(current)
        : new URLSearchParams(Object.entries(current).flatMap(([k, v]) => v === undefined
            ? []
            : (Array.isArray(v) ? v : [v]).map((s) => [k, String(s)])));
    const set = (key, value) => {
        if (value === null || value === "")
            out.delete(key);
        else
...
export function emptyQuery(spec) {
    return {
        q: "",
        facets: {},
