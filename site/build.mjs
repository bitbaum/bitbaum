...
${(packages.packages ?? []).slice(0, 3).map((p) => pkgCard(p, cfg.packages?.[p.slug], ventureBySlug, alias)).join("\n")}
    ...
    <section class="section" id="open">
      <div class="wrap">
        ...
        <div class="facts">
          <div class="fact"><span class="label">Licence</span><span class="display-3">MIT for the trunk</span><p class="copy">All ${pkgCount} shared packages and the fleet\u2019s own infrastructure are MIT \u2014 that is what &ldquo;take it&rdquo; applies to. <a href="/orangecat/">OrangeCat</a> and <a href="/loki/">Loki</a> are public to read but not openly licensed yet, and work built for someone else stays theirs.</p></div>
  ...
  const body = `  <main>
    ...
    <section class="section">
      <div class="wrap"><div class="prose">
        ...
        <p>The ${pkgCount} shared packages and the fleet\u2019s infrastructure are MIT. <a href="/orangecat/">OrangeCat</a> and <a href="/loki/">Loki</a> are not: their code is public to read, but their licence grants no rights, so &ldquo;take it&rdquo; means the trunk and not those two — an open decision, stated rather than glossed. Contributor terms live in one place, <a href="${CONTRIBUTING}">bitbaum/.github</a>: sign off your commits and a sweep merges anything green. Every repository\u2019s origin is stamped nightly through OpenTimestamps and archived by Software Heritage${block ? `, anchored in Bitcoin since block ${block}` : ""}, so precedence is arithmetic rather than a claim.</p>
